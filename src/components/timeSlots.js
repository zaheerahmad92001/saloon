...
const TimeSlots = (props) => {
  ...
  const toggleSelection = (slot) => {
    setSelectedTime((prevSelected) => {
      const isAlreadySelected = prevSelected.some((item) => item.id === slot.id);
      const newSelected = isAlreadySelected
        ? prevSelected.filter((item) => item.id !== slot.id)
        : [...prevSelected, slot];

      setSelectedSlots(newSelected); // Update parent state
      return newSelected;
    });
  };
  ...
  return (
    <View style={!isEdit ? styles.timePicker : styles.timePicker2}>
      {!isEdit && <Text style={styles.sectionTitle}>{title}</Text>}
      <View style={styles.times}>
        {availableSlots?.map((slot) => {
          const isSelected = isEdit
            ? selectedSlots?.some((s) => s.id === slot.id)
            : selectedTime?.some((s) => s.id === slot.id);
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => {
                isEdit ? handleSlotSelection(slot) : toggleSelection(slot);
              }}
            >
              <Text
                style={[
